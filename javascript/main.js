            ...
            try {
                ...
                if (ui.currentlyPlaying && ui.currentlyPlaying instanceof UnitCard) {
                    if (ui.currentlyPlaying.fieldPos) {
                        ...
                        on = game.cardsByPos(side, row, col)[0];
                        if (!on) {
                            while (!game.cardsByPos(side, row, col - 1).filter(x => x != ui.currentlyPlaying).length && col > 0)
                                col--;
                        }
                            ...
                            log("Uh oh.");
                        let slot = getId(`slot-${side}-${col}-${row}`);
                        ui.currentlyPlaying.fieldPos = { side: side, row: row, column: col };
